(function (global, factory) {
  ...
  (factory((global.todomvc = global.todomvc || {})));
...
var autosave = function (saveKey) { return function ( Base, store ) {
	store.subscribe( function ( action, state ) {
...
var todos = JSON.parse( window.localStorage.getItem( 'reo-todos' ) || '[]' );
var state = {
	todos: todos,
	filteredTodos: [],
	filter: 'all'
...
var todos$1 = {
	...
	reducers: {
		...
		delete: function delete$1( state, todo ) {
		...
		toggleAll: function toggleAll( state ) {
			var hasUndo = state.todos.some( function (todo) { return !todo.done; } );
			state.todos.forEach( function (todo) { return todo.done = hasUndo; } );
			doFilter( state );
		},
		clearCompleted: function clearCompleted( state ) {
			state.todos = state.todos.filter( function (todo) { return !todo.done; } );
			doFilter( state );
		},
		// filters
		filter: doFilter
...
function doFilter( state, filter ) {
	...
	switch( state.filter ) {
		case 'all':
			...
			break;
		case 'active':
			state.filteredTodos = state.todos.filter( function (todo) { return !todo.done; } );
			break;
		case 'completed':
			state.filteredTodos = state.todos.filter( function (todo) { return todo.done; } );
			break;
...
var actions = {
	add: function add( ref, text ) {
		var commit = ref.commit;
		var get = ref.get;

		commit( 'todos/add', text );
	},
	delete: function delete$1( ref, todo ) {
		var commit = ref.commit;
		var get = ref.get;

		commit( 'todos/delete', todo );
	},
	edit: function edit( ref, ref$1 ) {
		var commit = ref.commit;
		var get = ref.get;
		var todo = ref$1.todo;
		var text = ref$1.text;

		commit( 'todos/edit', { todo: todo, text: text } );
	},
	toggle: function toggle( ref, todo ) {
		var commit = ref.commit;
		var get = ref.get;
	...
	clearCompleted: function clearCompleted( ref ) {
		var commit = ref.commit;
		var get = ref.get;
	...
	// filters
	filter: function filter( ref, filter ) {
		var commit = ref.commit;
		var get = ref.get;
...
var getters = {
	filteredTodos: function (state) { return state.todos.filteredTodos; },
	todos: function (state) { return state.todos.todos; },
	leftCount: function (state) { return state.todos.todos.filter( function (todo) { return !todo.done; } ).length; },
	filter: function (state) { return state.todos.filter; },
};
